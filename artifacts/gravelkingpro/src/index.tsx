...
if (
  ...
  void navigator.serviceWorker.getRegistrations().then(async (registrations) => {
    const sameSite = registrations.filter((registration) => registration.scope.startsWith(window.location.origin));
...
async function checkForNewBuild(): Promise<void> {
  ...
  try {
    ...
    const response = await fetch(checkUrl, { cache: "no-store", headers: { "Cache-Control": "no-cache" } });
    if (!response.ok) return;
    const html = await response.text();
    const serverBuild = new DOMParser().parseFromString(html, "text/html")
      .querySelector('meta[name="gkp-build-version"]')?.getAttribute("content");
