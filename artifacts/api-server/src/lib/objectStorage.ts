...
export class ObjectStorageService {
  ...
  /** Persist generated bytes in the configured private namespace without
   * creating a transient signed URL. Callers that keep the key in PostgreSQL
   * can later authorize and stream it through their own endpoint. */
  async savePrivateBuffer(
    ...
    const base = privateDir.endsWith("/") ? privateDir.slice(0, -1) : privateDir;
    const cleanKey = key.replace(/^\/+/, "");
    const { bucketName, objectName } = parseObjectPath(`${base}/${cleanKey}`);
    await saveObjectWithFallback(bucketName, objectName, buffer, {
      contentType,
