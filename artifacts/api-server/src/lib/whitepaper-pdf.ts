 ...
 *
 * Mirrors the client-side jsPDF document on /whitepaper (gravelkingpro
 * frontend) so `GET /api/v1/download-whitepaper` streams the identical
 * 7-page brief: cover, five numbered sections, and a contact page.
 *
...
export function buildWhitepaperPdf(): Buffer {
  ...
  doc.line(MARGIN_L, 104, MARGIN_R, 104);
  ...
  const bullet = (text: string) => {
    ...
    doc.setFontSize(9.5);
    doc.setTextColor(161, 161, 170);
    const lines: string[] = doc.splitTextToSize(`• ${text}`, LINE_W - 4);
    lines.forEach((line, i) => {
      doc.text(line, MARGIN_L + (i > 0 ? 4 : 0), y);
      y += 5;
    });
  };
  ...
  body(
    "Runs ffmpeg ebur128 loudness analysis on the submitted audio. Extracts integrated LUFS (loudness units relative to full scale) and true peak dBFS from the Summary block of the EBU R128 report. Rejects:",
  );
  ...
  bullet(
    "Anchor A (Nominator) — embedded into the audio's least-significant bits. Encodes: SHA-256(contentHash | artistHandle | certId), truncated to 32 hex chars. Survives lossless export (WAV, AIFF, FLAC). Metadata-independent — travels with the signal, not the container.",
  );
  bullet(
    "Anchor B (Denominator) — stored exclusively on GravelKing servers. Never transmitted to any client. The server-side complement required to reconstruct and verify the full handshake.",
  );
  bullet(
  ...
  body(
    "POST /api/kernel/verify-signal accepts any WAV file and returns a structured VerifyResult: { warrant, anchorA, anchorB, hmac, certId, artist, certifiedAt }. Warrant status is one of:",
  );
  ...
  bullet(
    "POST /api/v1/ingest (alias: /api/kernel/master) — Submit WAV/FLAC/AIFF/MP3/AAC for MLK V3.5 processing + cert embedding. Returns processed audio stream + X-GK-CertId response header.",
  );
  bullet(
    "POST /api/v1/verify (alias: /api/kernel/verify-signal) — Public clean-room verification. No auth required. Returns full VerifyResult JSON. < 2ms median response for pre-certified tracks.",
  );
  ...
  bullet("GET  /api/v1/download-whitepaper — Stream this technical brief as a PDF document.");
  ...
  doc.setDrawColor(201, 162, 39);
  doc.setLineWidth(0.4);
  doc.line(MARGIN_L, 100, MARGIN_R, 100);
