#!/usr/bin/env node
/**
 * Export the GravelKing Pro Main Stage duet promo to MP4.
...
if (!output) {
  if (isLogo) {
    ...
    output = 'public/videos/gka_main_stage_duet_30s_16x9.mp4';
  }
}
...
async function mixAudio() {
  ...
  if (isWorkflow) {
    ...
    execFileSync('ffmpeg', [
      '-y', '-f', 'lavfi', '-i',
      'aevalsrc=0.045*sin(2*PI*55*t)+0.018*sin(2*PI*110*t)+0.008*sin(2*PI*220*t):s=48000:d=30',
      '-t', '30',
      '-filter_complex', `
        [0:a]pan=stereo|c0=c0|c1=c0,
        acompressor=threshold=-18dB:ratio=2.2:attack=8:release=120:makeup=2,
        loudnorm=I=-16:TP=-1.5:LRA=9,
        afade=t=in:st=0:d=0.5,afade=t=out:st=28.5:d=1.5[aout]
      `,
      '-map', '[aout]',
      '-c:a', 'aac', '-b:a', '256k', '-ar', '48000',
      audioPath,
