...
**MLK V4 (Morris Law Kernel V4) Python kernel:** Lives at `.local/mlk-repo/morris_law_kernel_v35_fast.py`. Requires ONLY numpy + scipy + psutil. Never needs torch/demucs/CUDA. The `demucs` audio separation path in `gkp-separator.ts` is dead-pathed — all audio processing uses in-process ffmpeg (MLK V4 (Morris Law Kernel V4) JS kernel). Keep `pyproject.toml` with only numpy/scipy/psutil declared.
...
Update (Aug 2026, second incident — the real lever): **`.replitignore` at the workspace root IS honored** (dockerignore format) and excludes paths from the packaged Repl layer, despite a docs-search answer claiming otherwise. It now excludes `.git`, `.local`, `.cache`, heavy `attached_assets` media, and dev-only folders. CRITICAL: excluded paths are absent in the deploy **builder** too — never exclude anything imported at build time (promo scenes import two `attached_assets/*.png`) or needed at runtime (`.pythonlibs`). Also: a full local `pnpm install` regrows `.local/share/pnpm` (1.3 GB) + `.cache/pnpm` (280 MB) — deleting the store outright is survivable (hardlinks keep node_modules intact) but `pnpm store prune` is the polite version.
