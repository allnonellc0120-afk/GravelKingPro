...
Update (later incident): root-level ad-hoc Python installs (torch/nvidia/triton/onnxruntime via pip/uv, NOT declared in pyproject.toml) land in `.pythonlibs` (~5 GB) and get bundled into the deploy image too. Check `du -sh .pythonlibs .cache/uv` before publishing; delete `.pythonlibs` whenever no workflow actually runs Python — UPM reinstalls only declared pyproject deps (~700 MB), so undeclared ML libs do not return. Do NOT delete `.local/share/pnpm` — it is the pnpm store backing node_modules hardlinks.
