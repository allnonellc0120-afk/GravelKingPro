...
**Why (gitignored caches):** The packaged Repl layer includes gitignored working-tree dirs — `node_modules`, `.cache/`, `.local/`. Gitignored does NOT mean excluded from the deploy image. The Python `uv` cache (`.cache/uv`) balloons to ~4–5 GiB after MLK-kernel Python installs and pushes the image over 8 GiB even with `ffmpeg-headless`. (Proof the deploy uses working-tree state, not a clean install: a deploy once failed on a module that was stale-missing only in the working-tree `node_modules` while the committed lockfile was correct — see `expo-deploy-node-modules.md`.)

**Fix (gitignored caches):** Before publishing, delete regenerable Python caches: `rm -rf .cache/uv .cache/torch .cache/pip`. They don't regrow from the Node-based workflows or the deploy build, so a one-time cleanup persists through the publish. Keep `.cache/pnpm` and `.cache/typescript` (small + useful). Measure first with `du -sh .cache node_modules .local` — these three plus the nix closure are what fill the 8 GiB budget.
