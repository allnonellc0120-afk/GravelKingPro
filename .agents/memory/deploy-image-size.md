...
Update (Aug 2026, ROOT CAUSE of the 12:56/13:28 failures): `pkgs.chromium` had been added to replit.nix as a system dependency for live-site browser proof — its ~1.5–2 GB closure inflated the nix layer and broke every subsequent publish even after 2 GB of workspace cleanup. NEVER install chromium (or other GUI/browser packages) via installSystemDependencies; use ad-hoc `nix-shell -p chromium` instead (see live-site-browser-proof.md). Diagnostic tell: build log says "Nix layers for this Repl are uncached" right before the size failure → the nix layer changed; diff replit.nix first, before cleaning workspace weight.

Update (same day, workspace weight): image crept over 8 GiB with NO single new cause — chronic weight. Safe quick wins in order of payoff: `pnpm store prune` (~1.4 GB), delete `artifacts/*/dist` (rebuilt at publish; promo video artifacts duplicate 250+ MB of mp4s into dist), `rm -rf .cache/pnpm`, `git gc --prune=now`. Structural fat that needs user sign-off: `attached_assets/` (500 MB of chat uploads incl. an .aab and screen recordings), duplicated 51 MB promo masters in BOTH gravelkingpro-promo and gravelkingpro-speed-promo public/videos, `.git/lfs` (276 MB, `git lfs prune` candidate). The whole workspace incl. `.git` ships in the Repl layer. Check headroom with `du -sh /home/runner/workspace` (target ≤ ~5.5 GB).
