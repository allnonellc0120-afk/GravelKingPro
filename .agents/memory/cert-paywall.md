...
- Monthly/Studio + node_auditor: 20 included unlocks per rolling 30 days (users.cert_unlocks/cert_unlock_period_start, same atomic reset-or-increment as exportQuota). Unlock claims the stub first (conditional on still-locked), then consumes the allowance, rolling back the claim if exhausted — so races never double-consume.
